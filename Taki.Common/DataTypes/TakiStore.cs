...
namespace Taki.Common.DataTypes
{
    ...
    public class TakiStoreNonSingleton
    {
        ...
        List<ITakiCard> _allPlusCards;
               
        public List<ITakiCard> GetAllPlusCards()
        {
            if (_allPlusCards == null)
            {
                _allPlusCards = new List<ITakiCard>();

                _allPlusCards.Add(new SpecialTakiCard(CardColors.Blue, SpecialCards.Plus));
                _allPlusCards.Add(new SpecialTakiCard(CardColors.Green, SpecialCards.Plus));
                _allPlusCards.Add(new SpecialTakiCard(CardColors.Red, SpecialCards.Plus));
                _allPlusCards.Add(new SpecialTakiCard(CardColors.Yellow, SpecialCards.Plus));

            }

            return _allPlusCards;
        }
        ...
        public List<ITakiCard> GetAllNumberCards()
        {
            if (_allNumberCards == null)
            {
                ...
                foreach (var card in GetAllCardsPackage(1))
                {
        ...
        public List<ITakiCard> CanFollow(ITakiCard LowestCard)
        {
            ...
            {
                if ((LowestCard as SpecialUniversalTakiCard).Type == SpecialUniversalCards.Shifter)
                {
                    return GetAllCardsPackage(1);
                }
                else
                {
        ...
        public List<ITakiCard> GetAllCardsPackage(int copies)
        {            
            if (_allCards == null)
            {
        ...
        public List<ITakiCard> GetShuffledCardsPackage(int copies)
        {
            //Shuffle It...
            return ShuffleList<ITakiCard>(GetAllCardsPackage(copies));
        }      
